...
function updateLoupe(x,y,e){
  ...
  const fallbackX = wrap.width > 250 ? 26 : 10;
  const fallbackY = wrap.height > 180 ? 26 : 10;
  const lx = e ? Math.min(Math.max(20, e.clientX-wrap.left+24), Math.max(20, wrap.width-210)) : fallbackX;
  const ly = e ? Math.min(Math.max(20, e.clientY-wrap.top+24), Math.max(20, wrap.height-165)) : fallbackY;
...
(function(){
  ...
  bind('enlargeBtn', ()=>{
    ...
    $('canvasWrap').classList.toggle('enlargeMode', enlargeMode);
    $('enlargeBtn').classList.toggle('active', enlargeMode);
    if(enlargeMode){ scale=Math.max(scale,1.8); applyScale(); updateLoupe(canvas.width/2, canvas.height/2, null); }
    else { if(loupe) loupe.hidden=true; fitCanvas(); }
  });
})();
