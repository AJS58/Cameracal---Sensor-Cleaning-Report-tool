...
function generateReportWindow(){
  ...
  const confidenceOrganic = (s.count === 0 || !s.meaningfulDetection) ? 0 : Math.min(95, Math.round((s.large*5) + (s.coverage*8) + (s.count>100?35:15)));
  const confidenceMoisture = (s.count === 0 || !s.meaningfulDetection) ? 0 : Math.min(88, Math.round((s.large*3) + (s.coverage*5) + (s.pattern.includes('moisture')?35:20)));
  const confidenceDry = (s.count === 0 || !s.meaningfulDetection) ? 0 : Math.max(5, Math.round(100 - confidenceOrganic/1.4));
  const noDetection = s.count === 0 || !s.meaningfulDetection;
  ...
  <div class="page">
    ...
    ${noDetection ? `
      <div class="cta">
        ...
        <p>No significant dust spots, bonded debris clusters or residue patterns were detected from the supplied image. Any tiny isolated marks detected are below the threshold required for reliable contamination classification.</p>
