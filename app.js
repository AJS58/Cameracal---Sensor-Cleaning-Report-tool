...
function apertureVisibility(s){
  ...
  const rows = [
    {ap:'f/4', risk: level({low:180, moderate:320, high:520, extreme:850}), note:'Only larger or bonded contamination is usually visible at wider apertures.'},
    {ap:'f/5.6', risk: level({low:120, moderate:240, high:420, extreme:700}), note:'Large spots, residue and bonded debris may appear on plain skies or backgrounds.'},
    {ap:'f/8', risk: level({low:60, moderate:120, high:220, extreme:520}), note:'Contamination can become noticeable on skies, studio backdrops and smooth tones.'},
    {ap:'f/11', risk: level({low:35, moderate:80, high:160, extreme:350}), note:'Dust visibility increases significantly on skies, white backgrounds and studio backdrops.'},
    {ap:'f/16', risk: level({low:15, moderate:40, high:90, extreme:200}), note:'This is a recommended dust-test aperture range and reveals most contamination.'},
    {ap:'f/22', risk: level({low:8, moderate:22, high:55, extreme:130}), note:'Small particles, faint marks and bonded residue become much more visible.'}
  ];
...
function generateReportWindow(){
  ...
  const cleanUrl=imageDataToDataUrl(cleanData);
  ...
  const html=`<!doctype html><html><head><title>Cameracal Sensor Health Report</title><style>
    ...
    h1{margin:0;color:#0057d8;font-size:34px} h2{color:#0057d8;margin-top:0}.blue{color:#0057d8}.red{color:#e00000}.orange{color:#f07800}
  ...
  <div class="page">
    ...
    <div class="grid">
      ...
      <div class="card">Estimated coverage<strong>${s.coverage.toFixed(2)}%</strong></div>
      <div class="card">Health score<strong>${s.healthScore}/100</strong></div>
    </div>
    <p>This report analyses sensor contamination visible under dust-revealing conditions. Results depend on the supplied image and shooting conditions.</p>
  </div>
  ...
  <div class="page">
    ...
    <div class="heatLegend">
      <div class="heatItem"><span class="heatSwatch" style="background:#ff0000"></span>Heavy concentration</div>
      <div class="heatItem"><span class="heatSwatch" style="background:#ff8c00"></span>Moderate concentration</div>
      <div class="heatItem"><span class="heatSwatch" style="background:#ffe600"></span>Light concentration</div>
    </div>
  ...
  <div class="page">
    ...
    <p>This paid report includes an estimated guide to how likely the detected contamination is to be visible at wider and smaller apertures. This is calculated from the supplied dust-test image, total spot count, estimated coverage and heavy contamination level.</p>
    <table><thead><tr><th>Aperture</th><th>Estimated Visibility Risk</th><th>Comment</th></tr></thead><tbody>${av.rows.map(r=>`<tr><td><b>${r.ap}</b></td><td><b>${r.risk}</b></td><td>${r.note}</td></tr>`).join('')}</tbody></table>
