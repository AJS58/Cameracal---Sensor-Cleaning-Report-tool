...
function summary(){
  ...
  let pattern='No significant contamination detected';
  if(count>0){
...
function generateReportWindow(){
  ...
  const confidenceOrganic = s.count === 0 ? 0 : Math.min(95, Math.round((s.large*5) + (s.coverage*8) + (s.count>100?35:15)));
  const confidenceMoisture = s.count === 0 ? 0 : Math.min(88, Math.round((s.large*3) + (s.coverage*5) + (s.pattern.includes('moisture')?35:20)));
  const confidenceDry = s.count === 0 ? 0 : Math.max(5, Math.round(100 - confidenceOrganic/1.4));
  ...
  <div class="page">
    ...
    ${noDetection ? `<p class="small"><b>No heat-map activity:</b> no measurable contamination clusters were detected, so no density zones are shown.</p>` : `<p class="small">Heat map visualisation estimates contamination density and cluster severity across the sensor area.</p>`}
  </div>
  ...
  <div class="page">
    ...
    ${noDetection ? `
      <div class="cta">
        ...
        <p>Contamination confidence classification has been suppressed because there is insufficient measurable evidence.</p>
      </div>
...
(function(){
  ...
  if (help) help.addEventListener('click', () => show('Help', `
    ...
    <p>Use a JPEG image taken at F16 / F22 of a plain bright subject. Defocus the lens slightly and avoid patterned backgrounds.</p><p>Avoid prominent clouds, trees, buildings or strong sky gradients, as these can confuse detection and may be mistaken for contamination.</p>
    <h3>Manual tools</h3>
